...
def emit_qrcode_event(qr_code: str) -> None:
    """Output login QR code as a structured stdout event for API services."""
    if "," in qr_code:
        qr_code = qr_code.split(",", 1)[1]

    payload = {
        "event": "login_qrcode",
        "platform": os.getenv("MEDIACRAWLER_TASK_PLATFORM", ""),
        "task_id": os.getenv("MEDIACRAWLER_TASK_ID", ""),
        "image_base64": qr_code,
        "image_data_url": f"data:image/png;base64,{qr_code}",
    }
    print(f"{QR_CODE_EVENT_PREFIX}{json.dumps(payload, ensure_ascii=False)}", flush=True)
...
def show_qrcode(qr_code) -> None:  # type: ignore
    ...
    if os.getenv("MEDIACRAWLER_QR_OUTPUT", "").lower() == "api":
        emit_qrcode_event(qr_code)
        return
