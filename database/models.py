...
from sqlalchemy import create_engine, Column, DateTime, Integer, Text, String, BigInteger
...
class VideoAccount(Base):
    __tablename__ = 't_video_account'

    id = Column(BigInteger, primary_key=True, autoincrement=True, comment='主键ID')
    nick_name = Column(String(255), comment='账号昵称')
    url = Column(String(512), comment='主页链接')
    ip_location = Column(String(255), comment='归属地')
    total_favorited = Column(Integer, comment='点赞量')
    short_id = Column(String(255), comment='抖音号')
    following_count = Column(Integer, comment='主动关注量')
    mplatform_followers_count = Column(Integer, comment='粉丝数')
    user_age = Column(Integer, comment='年龄')
    signature = Column(Text, comment='用户签名')
    third_user_id = Column(String(255), index=True, comment='三方用户id')
    avatar = Column(String(512), comment='头像')
    aweme_count = Column(Integer, comment='作品总数')
    create_time = Column(DateTime, comment='创建时间')
    create_by = Column(String(80), comment='创建人')
...
class VideoDetail(Base):
    __tablename__ = 't_video_detail'

    id = Column(BigInteger, primary_key=True, autoincrement=True, comment='主键ID')
    account_id = Column(BigInteger, comment='账号id')
    aweme_id = Column(String(255), index=True, comment='视频id')
    title = Column(String(255), comment='标题')
    nick_name = Column(String(255), comment='账号昵称')
    video_cover = Column(Text, comment='视频封面')
    work_type = Column(String(255), comment='作品类型')
    admire_count = Column(String(255), comment='赞赏次数')
    digg_count = Column(String(255), comment='点赞数')
    comment_count = Column(String(255), comment='评论数')
    collect_count = Column(String(255), comment='收藏数')
    share_count = Column(String(255), comment='分享数')
    view_count = Column(String(255), comment='播放量')
    duration = Column(String(255), comment='播放时长(秒)')
    topics = Column(String(255), comment='视频标签')
    video_create_time = Column(String(255), comment='视频创建时间')
    video_addr = Column(Text, comment='视频播放地址URL')
    images = Column(Text, comment='图片地址列表（图集作品的图片URL数组，视频作品为空列表）')
    user_url = Column(String(512), comment='作者主页链接')
    author_avatar = Column(Text, comment='作者头像URL')
    following_count = Column(String(255), comment='关注数量（作者关注的用户数）')
    follower_count = Column(String(255), comment='粉丝数量（作者的粉丝总数）')
    total_favorited = Column(String(255), comment='获赞总数（作者所有作品累计获得的点赞和收藏数）')
    aweme_count = Column(String(255), comment='作品数量（作者发布的作品总数）')
